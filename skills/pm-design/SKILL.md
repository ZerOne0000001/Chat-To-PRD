...
5. **阶段 4: 信息架构 (页面结构 IA)**:
   - **设备与端侧确认**: 在编写页面结构前，必须先向用户询问**设备分辨率/端口** (例如：移动端、桌面端、响应式、小程序)。等待用户确认后再继续。
   ...
   - **内容颗粒度要求 (严格)**: 
     ...
     - **页面分区**: 必须按照功能模块（如状态信息、核心操作区、数据列表区等抽象概念）进行拆解，**绝对不要带有明确的位置指向**（如顶部状态栏、底部操作栏、左侧等），以为下一阶段原型制作保留足够的创作空间。
